...
namespace TerminalRaid.Services
{
    ...
    {
        public async Task<string> InitializeSessionAsync(string username)
        {
            var bootMessages = new List<string>
            {
            ...
            var bootSequence = string.Join("\n", bootMessages);
            var prompt = $"{username}@{username}-PC:$~";
            return welcome + bootSequence + prompt;
        }
    }
}